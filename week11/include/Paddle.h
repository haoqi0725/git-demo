...
public:
    Paddle(float x, float y, float w, float h, float spd = 8.0f);
    
    void SetSpeed(float s) { speed = s; }
    void SetWidth(float w) { width = w; originalWidth = w; }
    void SetYPosition(float y) { position.y = y; }
    ...
    void SetEffectRemaining(float t) { effectRemainingTime = t; }
};
